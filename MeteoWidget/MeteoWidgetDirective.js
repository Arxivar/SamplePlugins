angular.module('arxivar.plugins.directives').directive('meteowidgetdirective', ['MeteoWidget', 'pluginService', '_', function(MeteoWidget, pluginService, _) {
    return {
        restrict: 'E',
        scope: {
            instanceId: '@',
        ...
        link: function(scope, element) {
            ...
            var setUserSettings = function() {
                pluginService.setPluginByUser({ pluginId: MeteoWidget.plugin.id, desktopId: scope.desktopId, instanceId: scope.instanceId }, [
                    { name: 'citta', value: scope.selectedCity },
                    { name: 'selectedColor', value: scope.selectedColor },
                ]);
            };
            ...
            scope.colors = [
                ...
                { name: 'orange', value: '#F6981E' },
                { name: 'green', value: '#7BB77B' },
            ];
            ...
            var init = function() {
                ...
                pluginService.getPluginByUser({ pluginId: MeteoWidget.plugin.id, desktopId: scope.desktopId, instanceId: scope.instanceId }).then(function(settings) {
                    if (_.isNil(settings) || _.isNil(settings.userSettings)) {
                        scope.selectedCity = 'Montichiari';
                        scope.selectedColor = scope.colors[0].value;
                    } else {
                        scope.selectedCity = _.some(settings.userSettings, { name: 'citta' }) ? _.find(settings.userSettings, { name: 'citta' }).value : 'Montichiari';
                        scope.selectedColor = _.some(settings.userSettings, { name: 'selectedColor' }) ? _.find(settings.userSettings, { name: 'selectedColor' }).value : scope.colors[0].value;
                    }
                    searchWeather(scope.selectedCity);
