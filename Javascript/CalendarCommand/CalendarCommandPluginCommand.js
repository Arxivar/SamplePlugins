angular.module('arxivar.plugins').factory('CalendarCommand', ['$q', '$uibModal', 'PluginCommand', function($q, $uibModal, PluginCommand) {
	...
	var requiredSettings = {
		id: '086ef2a8-99de-4360-83b4-8c867a10b349', // Unique plugin identifier (type: string)
			...
			.then(function(canRun) {
				if (canRun) {
					...
					var modal = $uibModal.open({
						...
						controller: ['$scope', '$uibModalInstance', 'mediatorService', 'moment', 'arxivarResourceService', '_', 'arxivarNotifierService',
							function($scope, $uibModalInstance, mediatorService, moment, arxivarResourceService, _, arxivarNotifierService) {
								...
								$scope.confirm = function() {
									return $q.all([
										arxivarResourceService.get('profiles/Additional/' + classe + '/0/0'),
										arxivarResourceService.get('profiles/' + params.docnumbers[0] + '/schema/false')
									])
										.then(function(result) {
											var additionals = result[0]
											var data = result[1]
											//Cambio la classe
											_.find(data.fields, {
											...
											return arxivarResourceService.update('profiles/' + params.docnumbers[0], profile)
												.then(function() {
													$uibModalInstance.close();
													arxivarNotifierService.notifySuccess('Appuntamento salvato');
												}).catch(function(err) {
