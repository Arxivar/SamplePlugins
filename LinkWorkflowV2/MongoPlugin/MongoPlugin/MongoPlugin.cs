...
{
	[Plugin("47643477-b5a0-4154-b253-1a3511c7799b", "Mongo Query", "1.0.1", Description = "A plugin to interact with a Mongo database", Icon = "far fa-database", UseAdvancedConfiguration = true)]
	public class MongoPlugin : WorkflowPluginLink, IWorkflowPlugin
	{
		#region Constants

		private const string DatabaseName_ParameterName = "DatabaseName";
		private const string QueryText_ParameterName = "QueryText";
		private const string Columns_ParameterName = "Columns";
		private const string UseAdvancedConfiguration_ParameterName = "UseAdvancedConfiguration";
		private const string OutputVariableName_ParameterName = "OutputVariableName";
		...
		[Injected]
		public IMongoDbProvider MongoDbProvider { get; set; }

		[Injected]
		public IDiagramsApi DiagramsApi { get; set; }

		[Injected]
		public IProcessesApi ProcessApi { get; set; }

		[Injected]
		public IProcessVariablesApi ProcessVariablesApi { get; set; }
		...
		/// </summary>
		private async Task<object[,]> ParseOutputParameter(string jsonResult)
		{
			...
			object[,] matrix = BuildMatrixFromResult(parsedResult);
		...
		private void ValidateAdvancedConfiguration(IDictionary<string, object> advancedInput)
		{
			...
			if (!advancedInput.ContainsKey(UseAdvancedConfiguration_ParameterName))
			{
				throw new ArgumentException($"{UseAdvancedConfiguration_ParameterName} parameter cannot be empty");
			}

			if (!advancedInput.ContainsKey(Columns_ParameterName))
			{
				throw new ArgumentException($"{Columns_ParameterName} parameter cannot be empty");
			}

			if (!advancedInput.ContainsKey(OutputVariableName_ParameterName))
			{
				throw new ArgumentException($"{OutputVariableName_ParameterName} parameter cannot be empty");
			}
		}
		...
		private void LoadAdvancedConfiguration(IDictionary<string, object> advancedInput)
		{
			...
			AdvancedConfigurationEnabled = Convert.ToBoolean(advancedInput[UseAdvancedConfiguration_ParameterName]);
			ColumnSelection = advancedInput[Columns_ParameterName].ToString();
			OutputVariableName = advancedInput[OutputVariableName_ParameterName].ToString();
		}

		private void ValidateCommandRequest(IDictionary<string, object> advancedInput)
		{
			if (!advancedInput.ContainsKey(DatabaseName_ParameterName))
			{
				throw new ArgumentException($"{DatabaseName_ParameterName} parameter cannot be empty");
			}

			if (!advancedInput.ContainsKey(QueryText_ParameterName))
			{
				throw new ArgumentException($"{QueryText_ParameterName} parameter cannot be empty");
			}
		}

		private void LoadCommandParameters(IDictionary<string, object> advancedInput)
		{
			DatabaseName = advancedInput[DatabaseName_ParameterName].ToString();
			QueryText = advancedInput[QueryText_ParameterName].ToString();
		}
